...
function executeCloseMonth() {
  ...
  renderAll();
  renderDashboard();
  showToast(`${y}年${m}月の${targets.length}台をアーカイブしました`);
}
...
function toggleArchiveMonth(head) {
  const month = head.closest('.arc-month');
  const cars = month.querySelector('.arc-cars');
  const open = month.dataset.monthOpen === '1';
  month.dataset.monthOpen = open ? '0' : '1';
  cars.style.display = open ? 'none' : 'block';
}
